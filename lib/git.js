...
export async function getCommitDetails(repoPath, hash) {
  ...
  if (cached) return cached;
  const resolved = resolveRepoPath(repoPath);
  const meta = await runGit(['show', '-s', '--date=iso', '--format=%H|%an|%ae|%ad|%s|%P', hash], resolved, {
    maxBuffer: 64 * 1024
  });
  const [fullHash, authorName, authorEmail, date, subject, parents] = meta.trim().split('|');
  const parentHash = parents ? parents.split(' ')[0] : null; // Get first parent
  ...
  const result = { hash: fullHash, parentHash, authorName, authorEmail, date, subject, changedFiles, diff, diffTooLarge: diff === null };
  setCache(key, result);
  return result;
}
...
export async function getTree(repoPath, hash, treePath = '') {
  ...
  const cached = getCache(key);
  if (cached) {
    console.log('[Git] getTree cache hit:', key);
    return cached;
  }
  const resolved = resolveRepoPath(repoPath);
  const treeRef = safePath ? `${hash}:${safePath}` : hash;
  const args = ['ls-tree', treeRef];
  console.log('[Git] Running git command:', 'git', args.join(' '), 'in', resolved);
  const output = await runGit(args, resolved, { maxBuffer: 256 * 1024 });
  console.log('[Git] Git output length:', output.length);
  ...
  console.log('[Git] Parsed entries:', entries.length);
