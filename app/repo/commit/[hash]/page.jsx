        ...
        <div>
          ...
          <div className="mb-4 space-y-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="font-mono text-sm font-semibold">{commit.hash}</h1>
            ...
            <div className="space-y-1 text-sm text-slate-700 dark:text-slate-300">
              ...
              <p>
                {commit.authorName} &lt;{commit.authorEmail}&gt; on {new Date(commit.date).toLocaleString()}
              </p>
            </div>
            <div className="flex gap-2">
              <Link
                href={`/repo/tree/${hash}?repoPath=${encodeURIComponent(repoPath)}`}
          ...
          <div className="flex gap-4">
            ...
            >
              <div className="sticky top-20 rounded-lg border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                {/* Sidebar Header */}
                <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-800">
                  {!sidebarCollapsed && (
                ...
                {!sidebarCollapsed && (
                  <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
                    <button
                      ...
                      className={`flex w-full items-center gap-2 border-b border-slate-200 px-3 py-2 text-xs hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800 ${
                        selectedFile === null ? 'bg-blue-50 dark:bg-blue-950' : ''
                      }`}
                    >
                      <span className="font-semibold text-slate-700 dark:text-slate-300">All Files</span>
                    </button>
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <DiffViewer diff={commit.diff} selectedFile={selectedFile} />
            </div>
          </div>
        </div>
      )}
